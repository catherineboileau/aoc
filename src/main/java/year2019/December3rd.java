...
public class December3rd extends InputReader {
	 ...
	 * December 3rd, Part 2
	 */
	public Integer manhattanShortestSignal() {
		final List<PairWithSignal> wire1 = getCoordinatesWithSignalForWire(wire1Tab);
		final List<PairWithSignal> wire2 = getCoordinatesWithSignalForWire(wire2Tab);

		List<Integer> signals = wire1.stream().map(pairWithSignal -> {
			if (wire2.contains(pairWithSignal)) {
				PairWithSignal fromWire2 = wire2.get(wire2.indexOf(pairWithSignal));
				int signal = pairWithSignal.signal + fromWire2.signal;
				return new PairWithSignal(pairWithSignal.pair, signal);
			}
			
			return null;
		})
				.filter(pairWithSignal -> Objects.nonNull(pairWithSignal))
				.map(pairWithSignal -> pairWithSignal.signal)
				.collect(Collectors.toList());

		return Collections.min(signals);
	}

	private Set<Pair<Integer, Integer>> getCoordinatesForWire(String[] wire1Tab) {
		Pair<Integer, Integer> origin = new Pair<>(0, 0);
		Set<Pair<Integer, Integer>> wire1 = new HashSet<>();

		for (int j = 0; j < wire1Tab.length; j++) {
			String instruction = wire1Tab[j];
			Integer number = Integer.valueOf(instruction.substring(1));
			Set<Pair<Integer, Integer>> wireTemp = new HashSet<>();

			if (instruction.startsWith("R")) {
				for (int i = 0; i <= number; i++) {
					Pair<Integer, Integer> pair = new Pair<>(origin.getValue0() + i, origin.getValue1());
					wireTemp.add(pair);
				}

				origin = new Pair<>(origin.getValue0() + number, origin.getValue1());
			}
			...
			if (instruction.startsWith("U")) {
				for (int i = 0; i <= number; i++) {
					Pair<Integer, Integer> pair = new Pair<>(origin.getValue0(), origin.getValue1() + i);
					wireTemp.add(pair);
				}

				origin = new Pair<>(origin.getValue0(), origin.getValue1() + number);
			}
			...
			wire1.addAll(wireTemp);
		}

		return wire1;
	}

	private List<PairWithSignal> getCoordinatesWithSignalForWire(String[] wire1Tab) {
		Pair<Integer, Integer> origin = new Pair<>(0, 0);
		List<PairWithSignal> wire1 = new ArrayList<>();
		Integer countSteps = 1;

		for (int j = 0; j < wire1Tab.length; j++) {
			String instruction = wire1Tab[j];
			Integer number = Integer.valueOf(instruction.substring(1));
			List<PairWithSignal> wireTemp = new ArrayList<>();

			if (instruction.startsWith("R")) {
				for (int i = 1; i <= number; i++) {
					Pair<Integer, Integer> pair = new Pair<>(origin.getValue0() + i, origin.getValue1());
					wireTemp.add(new PairWithSignal(pair, countSteps++));
				}
	...
	public static void main(String[] args) throws IOException {
		...
		System.out.println("Minimum distance: " + obj.manhattanShortestDistance());
		System.out.println("Minimum signal: " + obj.manhattanShortestSignal());
	}
}
